...
public class LineEncodings implements GLEventListener{
...
static void longestCommon(String Finalstr) {
		int max=0;
	int temp=0;
...
public static void main(String[] args) {
	...
	int option=sn.nextInt();
     ...
     System.out.println("Enter 1 if you want a fixed subsequence otherwise press 2");
    int k=sn.nextInt();
	...
	if(k==1) {
		if(k1==8)
	Finalstr=Finalstr.substring(0,length/2)+"00000000"+Finalstr.substring(length/2);
		else
			Finalstr=Finalstr.substring(0,length/2)+"0000"+Finalstr.substring(length/2);
		...
		//LPS
				String str=String.valueOf(Finalstr);
				...
				System.out.println("5. Alternate Mark Inversion (AMI)");
		        System.out.println("6. Scrambled AMI ");
				choice=sn.nextInt();
				encodeData(Finalstr,choice);
				...
				final GLProfile profile = GLProfile.get(GLProfile.GL2);
			      ...
			      LineEncodings l = new LineEncodings();
			      glcanvas.addGLEventListener(l);
			      
			      glcanvas.setSize(1500, 600);
			      JFrame.setDefaultLookAndFeelDecorated(true);
			      //creating frame
			      final JFrame frame = new JFrame ("OUPUT");
